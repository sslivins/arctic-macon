...
{
    for (size_t i = 0; i < MACON_FAULT_REGS_COUNT; ++i) {
        if (MACON_FAULT_REGS[i] == address) return true;
    }
    return false;
}
...
char *register_format_value(uint16_t address, uint16_t raw, char *buf, size_t buf_len)
{
    ...
    if (register_is_fault(address) || address == REG_STATUS_BYTE) {
...
// Decode a Macon fault bitfield register from the canonical macon_faults table:
// list the code (and label) for each active bit, then append the raw byte so
// any undecoded bit stays visible.
static int format_fault(uint16_t address, uint16_t raw, char *buf, size_t buf_len)
{
    if (raw == 0) {
    ...
    size_t nbits = 0;
    const MaconFaultBit *bits = macon_fault_bits_for_reg(address, &nbits);

    int off = 0;
    bool first = true;
    for (size_t i = 0; i < nbits && (size_t)off < buf_len - 1; ++i) {
        const MaconFaultBit &fb = bits[i];
        if (!(raw & (1u << fb.bit))) continue;
        int w;
        if (fb.label && fb.label[0]) {
            w = snprintf(buf + off, buf_len - off, "%s%s:%s",
                         first ? "" : " | ", fb.code, fb.label);
        } else {
            w = snprintf(buf + off, buf_len - off, "%s%s",
                         first ? "" : " | ", fb.code);
        }
        if (w > 0) off += w;
        first = false;
    ...
    if (first) {
        // No named bit matched, but raw != 0 — show only the raw byte below.
        off = 0;
    }
    if ((size_t)off < buf_len - 1) {
        off += snprintf(buf + off, buf_len - off, "%s(0x%02X)",
                        first ? "" : " ", raw & 0xFF);
    }
    return off;
}
...
{
    if (address == REG_STATUS_BYTE) {
        return format_bits(s_status_bits, sizeof(s_status_bits)/sizeof(s_status_bits[0]),
                           raw, buf, buf_len);
    }
    if (register_is_fault(address)) {
        return format_fault(address, raw, buf, buf_len);
    }
